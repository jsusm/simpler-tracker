...
import { activities, metrics, metricsEnumValues, qualitativeMetricLabels } from "#/db/schema";
...
export const createActivityAndMetricsSF = createServerFn()
  ...
  .handler(async ({ data }) => {
    try {
      const insertActivityResult = await db.insert(activities).values(data).returning({ id: activities.id });
      ...
      if (insertMetricsResult.length !== data.metrics.length) {
        throw new Error("Inserted Metrics and input metrics does not have the same length")
      }
      ...
      for (let i = 0; i < insertMetricsResult.length; i++) {
        if (data.metrics[i].type === "qualitative") {
          const insertions = data.metrics[i].qualitativeLabels?.map((l, idx) => ({ label: l, order: idx, metricId: insertMetricsResult[i].id }))
