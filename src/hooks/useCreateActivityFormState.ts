...
export function CreateActivityStepFormReducer(state: CreateActivityStepFormStateType, action: CreateActivityStepFormActionType): CreateActivityStepFormStateType {
  if (action.type === "setActivityDone") {
    ...
    return {
      ...state,
      data: { ...state.data, title: action.payload.title, description: action.payload.description },
  ...
  if (action.type === 'addMetricDone') {
    if (state.stepState.updateMetricIdx === undefined) {
      return {
        ...state,
        data: { ...state.data, metrics: [...state.data.metrics, { ...action.payload }] },
    ...
    return {
      ...state,
      data: { ...state.data, metrics: state.data.metrics.map((m, idx) => idx === state.stepState.updateMetricIdx ? { ...action.payload } : m) },
      stepState: { updateMetricIdx: undefined, state: "checkout" },
      history: [...state.history, state.stepState]
    }
  }
  if (action.type == "removeMetric") {
    ...
    return {
      ...state,
      data: { ...state.data, metrics: state.data.metrics.filter((_, idx) => idx !== action.payload.idx) },
      stepState: { updateMetricIdx: undefined, state: nextStep },
      history: state.history.filter(e => e.updateMetricIdx !== action.payload.idx)
    }
  }
  ...
  if (action.type == "removeMetricDone") {
    ...
    return {
      ...state,
      data: { ...state.data, metrics: state.data.metrics.filter((_, idx) => idx !== state.stepState.updateMetricIdx) },
      stepState: { updateMetricIdx: undefined, state: "checkout" },
      history: state.history.filter(e => e.updateMetricIdx !== state.stepState.updateMetricIdx)
    }
  }
...
export function getDefaultActivityMetricValuesFromState(state: CreateActivityStepFormStateType) {
  let defaultValues = {
    label: '',
    type: '',
  ...
  if (state.stepState.updateMetricIdx !== undefined) {
    let stateMetric = state.data.metrics[state.stepState.updateMetricIdx]
    if (stateMetric) {
      return stateMetric
    }
  }
...
export type CreateActivityDispatcherType = ActionDispatch<[action: CreateActivityStepFormActionType]>;
